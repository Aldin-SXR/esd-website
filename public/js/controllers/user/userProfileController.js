const userProfileController = ($scope, $http, toastr) => {
    $scope.loading = false;
    $scope.departments = [
    ...
    $scope.getProfile = () => {
        ...
        $http.get("/private/profile/" + token._id, HTTP_CONFIG_USER).then(response => {
    ...
    $scope.updateProfile = () => {
        ...
        /* Send update request */
        $http.put("/private/profile/" + token._id, $scope.editableProfile, HTTP_CONFIG_USER).then(response => {
            $scope.loading = false;
